...
class SampleSheet:
    ...
    def update_samplesheet(self):
        ...
        self.content[[self.fw_col, self.rev_col]] = self.content[[self.fw_col, self.rev_col]].apply(
            lambda x: self._fetch_filepath(x)
        )
    ...
    def write_samplesheet(self):
        self.content.to_csv("{}.tsv".format(
    ...
    def _fetch_filepath(self, read_files):
        ...
        def simplify_samplenames(filenames:pd.Series)-> pd.Series:
            return filenames.apply(lambda x: x.lower().split(".")[0].split("/")[-1].strip())
        ...
        def find_matching_file(filename, rootdir):
            ...
            raise FileNotFoundError(f"Could not locate the read specified as {filename}")
