...
class SampleSheet:
    def __init__(
        ...
        # Validation input
        if sample_col == None: sample_col = DEF_SAMPLE_ID
        if fw_col == None: fw_col = DEF_FW_READS
        if rev_col == None: rev_col = DEF_RV_READS
        if run_id == None: run_id = DEF_RUN
    ...
    def update_samplesheet(self):
        ...
        # Generate uqid
        self.content["uqid"] = (
            self.content[["run_id", self.fw_col, self.rev_col]].astype(str).agg("".join, axis=1)
            .map(generate_uqid)
        )
    ...
    def update_sampledb(self):
        ...
        if Path(self.sample_db_samplesheet).exists():
            ...
            output = pd.concat([db_content,
                self.content[
                    ~self.content["uqid"].isin(db_content["uqid"])
                ]
            ])
    ...
    def _fetch_filepath(self, inp_files, absolute=False, is_assembly=False):
        ...
        if is_assembly: rt = os.path.join(self.root_dir, "results")
        else: rt = os.path.dirname(self.filename)
